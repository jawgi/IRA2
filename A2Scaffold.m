classdef A2Scaffold < handle
    ...
    methods
        function self = A2Scaffold() 
            ...
            while(1)                   
                if ~self.systemStatus                           % System on standby
                    ...
                    elseif self.CheckStart()                      % System start/resume selected
                    ...
                    if self.dobotGoalsCompleted < self.numFruits
                        ...
                        if ~self.dobotStatus % false = picking up, true = placing down
                            fruitStartPoints = self.FruitPointCloud(self.dobotGoalsCompleted+1,10);                                                                                 %Getting sample of points on surface of fruit to give better pose options
                            ...
                            % dobotGoalReachable = self.nextDobotGoal{1};         %for debugging
                    ...
                    % Same as above for rebel but only start if dobot has completed at least 2 goals
                    if self.rebelGoalsCompleted < self.numFruits && self.dobotGoalsCompleted > 1
                        if ~self.rebelStatus
                            ...
                            % rebelGoalReachable = self.currentRebelGoal{1};        %for debugging
                            ...
                            % rebelGoalReachable = self.nextRebelGoal{1};           %for debugging
                    ...
                    if self.dobotGoalsCompleted >= self.rebelGoalsCompleted+2 || self.rebelGoalsCompleted == self.numFruits-1                               %while there's still rebel goals to complete
                        if rebelErrorDist < 0.05            % To move to trajectory without re-calculating if close to goal - speeds up process
                        ...
                        if rebelReached
                            switch self.rebelStatus
                                case 1
                                    ...
                                    disp(['rebel goals done = ', num2str(self.rebelGoalsCompleted)]);
                                case 0
                                    self.rebelStatus = 1; % Now placing down
                            end
                            if self.rebelGoalsCompleted == 9
                                self.MoveRobot('rebel',self.defaultDeltaQ,self.dobotGoalsCompleted,self.resetQ,'basic',-1);
                            end
                        end
                    end

                    if self.dobotGoalsCompleted ~= self.numFruits                   %Until dobot picking and placing completed
                        if dobotErrorDist < 0.05                                                    % To move to trajectory without re-calculating if close to goal - speeds up process
                            self.MoveRobot('dobot',length(self.dobotQMatrix)-1,self.dobotGoalsCompleted+1,self.dobotQMatrix,'advanced',self.dobotStatus);
                        else
                            self.MoveRobot('dobot',self.defaultDeltaQ, self.dobotGoalsCompleted+1,self.dobotQMatrix,'advanced',self.dobotStatus);       %default mode just needs interval of steps in stored qMatrix
                        end
                        ...
                        if dobotReached
                            switch self.dobotStatus
                                case 1
                                    ...
                                    disp(['dobot goals done = ', num2str(self.dobotGoalsCompleted)]);
                                case 0
                        ...
                        self.MoveRobot('dobot',self.defaultDeltaQ,self.dobotGoalsCompleted,self.resetQ,'basic',-1);
                    end
        ...
        function RunBasicMode(self)
            while(~self.taskComplete)
                for i = 2:self.numFruits                
                    ...
                    dobotStartQ = self.CalcJointStates(self.dobotModel,fruitStartPt,self.defaultSteps,'quintic','other');
                    ...
                    dobotMidQ = self.CalcJointStates(self.dobotModel,fruitMidQ,self.defaultSteps,'quintic','basic');
                    ...
                    dobotResetQ = self.CalcJointStates(self.dobotModel,self.resetQ,self.defaultSteps,'quintic','basic');
                    ...
                    rebelMidQ = self.CalcJointStates(self.rebelModel,fruitMidQ,self.defaultSteps,'quintic','basic');
                    ...
                    rebelDropQ = self.CalcJointStates(self.rebelModel,fruitDropQ,self.defaultSteps,'quintic','basic');
                    ...
                    rebelResetQ = self.CalcJointStates(self.rebelModel,self.resetQ,self.defaultSteps,'quintic','basic');
        ...
        function SimulateEnvironment(self, pointCloudOn)            
            ...
            self.environmentCl = [tableCl; bucketsCl; shootCl]; %with shoot
            if pointCloudOn
                plot3(self.environmentCl(:,1),self.environmentCl(:,2),self.environmentCl(:,3),'r.');
            end
        ...
        %% Outputs start position point cloud for specified fruit at a sample size - to help with reachable poses for robot picking up
        function fruitPtCl = FruitPointCloud(self, index, sampleQty)
            fruitIndex = index;

            surfacePtCl = self.allFruits.pointCloud{fruitIndex};
            % ptEnd = surfacePtCl(end,:);                               %for debugging
            ...
            for i = 2:interval:sizePtCl
                fruitPtCl = [fruitPtCl;surfacePtCl(i,:)];               % Add new fruit surface point to fruit point cloud
            end

            fruitStartTr = self.allFruits.startPoint{fruitIndex}.T;
            fruitPtCl = [fruitPtCl;fruitStartTr(1:3,4)'];
            % newPtEnd = fruitPtCl(end,:);                          %for debugging

            % sizeFinalPtCl = length(fruitPtCl);                    %for debugging
        end
        ...
        function reachablePoses = CheckReachable(self,robotModel,points,animateOn)
            ...
            for i = 1:numPoints(1)
                ...
                while errorDistance > self.maxGoalDistError && attempt < maxAttempt
                    ...
                    try
                        [Q1,~] = robotModel.ikcon(attemptTr, currentQ);
                        ...
                        if distQ2 > distQ1                  %i.e. if qmincon didn't help for lower error
                            finalQ = Q1;
                        else
                            finalQ = Q2;
                        end
        ...
        function [plyPlotted, nextFruitPt] = ReplotFruit(self, index,transform)
            ...
            fruitHandle = self.allFruits.handle{fruitIndex};              % getting handle of fruit
             ...
             if ~isempty(fruitHandle) && ishandle(fruitHandle)
                nextFruitPt = nextFruitTr(1:3,4)';                              % next point for fruit
                plyPlotted =  self.allFruits.plotFruitPly(fruitIndex,'moving',nextFruitTr);     % plotting fruit again
                drawnow limitrate;           
                pause(0.001);
             else
                plyPlotted = false;
        ...
        function moved = MoveRobot(self,robotName,deltaQ,index,qMatrix,mode,stage) 
            fruitIndex = index;
            moved = false;
            switch robotName
                case 'dobot'
        ...
        function dropped = DropFruit(self,fruitIndex,stage)
            ...
            switch stage
                ...
                case 'drop'
                    ...
                    finalDropPoint = [fruitDropPoint(1,1),fruitDropPoint(1,2),dropHeight];
                otherwise
                    error("Invalid drop stage chosen. Fruit point not updated.");
            end
            ...
            while currentHeight > stopHeight
                ...
                [~,~] = self.ReplotFruit(fruitIndex,nextFruitTr);
        ...
        function collision = InCollision(self)                  
            ...
            self.dobotEllipsoidPts = self.CreateLinkEllipsoids(self.dobotModel);             % Get ellipsoid link points for dobot
            self.rebelEllipsoidPts = self.CreateLinkEllipsoids(self.rebelModel);               % Get ellipsoid link points for rebel
            allRobotPts = [self.dobotEllipsoidPts;self.rebelEllipsoidPts];                          % Combine all robot link points
            
            % Go through each robot link point and compare to every hundredth environment point cloud point
            for i = 1:length(allRobotPts)
                robotPt = allRobotPts(i,:);
                for j = 1:100:size(self.environmentCl, 1)

                    environmentPt = self.environmentCl(j, :);
                    dist = self.dist2pts(environmentPt,robotPt);        %Check the distance between the robot point and environment point
                    
                    if dist <= self.maxGoalDistError                         
                        collision = true;                                                % If close to collision return immediately.
                        return;
                    end
                end
            end
    ...
    methods(Static)
        ...
        function status = EStopPressed(press)                          % NEED TO ADD FULL HARDWARE FUNCTIONALITY 
            status = press;
            if status
                input("Stop?");
        ...
        function  robotEllipsoids = CreateLinkEllipsoids(robotModel)
            ...
            for i = 1:robotModel.n
                ...
                % Getting radii for ellipsoid
                if L(i).d == 0 % One of the revolute links
                    radii = [max(0.01, abs(L(i).a)), linkDiameter, linkDiameter]; % X-axis
                else % If it's the prismatic link
                ...
                robotEllipsoids = [X(:), Y(:), Z(:)];
            end
        end

        %% Checks if start/resume button has been pressed
        function status = CheckStart()                                      % NEED TO ADD FULL HARDWARE FUNCTIONALITY 
        ...
        function dist=dist2pts(pt1,pt2)                 %From tutorial content
            % If 2D
            if size(pt1,2) == 2
